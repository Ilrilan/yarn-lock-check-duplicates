...
describe('Check script which search duplicates in package-lock files', function () {
	const bin = resolve(__dirname, '../../../bin/package-lock-check-duplicates.js');

	describe('Invalid case', () => {
		it('We check that duplicates will be found', function () {
			exec(`PATH_TO_FILE="/tests/spec/check-package/resource/package-lock.json" node ${bin} -s @tinkoff-codeceptjs`, { cwd: process.cwd() }, (err, stdout) => {
		...
		it('We check that if the path to the file is incorrect, the error falls', function() {
			exec(`PATH_TO_FILE="/tests/spec/check-package/package-lock.json" node ${bin} -s @tinkoff-codeceptjs`, { cwd: process.cwd() }, (err, stdout, stderr) => {
				assert.equal(err?.code, 1);
				assert.ok(stderr.includes(`Can't find "package-lock" file`));
			});
		});
		...
		it('We check without specifying the scope', function() {
			exec(`PATH_TO_FILE="/tests/spec/check-package/resource/package-lock.json" node ${bin}`, { cwd: process.cwd() }, (err, stdout, stderr) => {
				assert.equal(err?.code, 1);
				assert.ok(stderr.includes('Scope for search duplicates is not defined! Example: package-lock-check-duplicates -s @babel'))
			});
		});
	});
	...
	describe('Valid case', () => {
		it('We check with the file without repetitions', function() {
			exec(`PATH_TO_FILE="/tests/spec/check-package/resource/valid-package-lock.json" node ${bin} -s @tinkoff-codeceptjs`, { cwd: process.cwd() }, (err, stdout, stderr) => {
